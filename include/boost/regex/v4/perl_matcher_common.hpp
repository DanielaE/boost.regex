...
bool perl_matcher<BidiIterator, Allocator, traits>::match_imp()
{
   ...
   m_presult->set_size((m_match_flags & match_nosubs) ? 1 : 1 + static_cast<unsigned>(re.mark_count()), search_base, last);
...
bool perl_matcher<BidiIterator, Allocator, traits>::find_imp()
{
   ...
   if((m_match_flags & regex_constants::match_init) == 0)
   {
      ...
      m_presult->set_size((m_match_flags & match_nosubs) ? 1 : 1 + static_cast<unsigned>(re.mark_count()), base, last);
      m_presult->set_base(base);
      m_presult->set_named_subs(this->re.get_named_subs());
   ...
   if(m_match_flags & match_posix)
   {
      m_result.set_size(1 + static_cast<unsigned>(re.mark_count()), base, last);
